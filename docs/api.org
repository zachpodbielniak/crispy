...
Context structure passed to every hook function. Contains both read-only pipeline state and mutable fields that plugins can modify to alter execution behavior. See [[file:plugins.org][docs/plugins.org]] for full field reference.
...
See [[file:config.org][docs/config.org]] for the full configuration guide.
