...
:PROPERTIES:
:CUSTOM_ID: plugin-authoring-guide
:END:
...
Each plugin is opened with =G_MODULE_BIND_LOCAL=: every plugin exports
the same symbol names, so its own are kept out of the process-wide
namespace that the loaded script also lives in. A plugin therefore
cannot publish anything to a script, or to another plugin, by exporting
a symbol -- the shared data store
(=crispy_plugin_engine_set_data()= / =_get_data()=) is the supported way
for plugins to reach each other.
...
** Plugin Contract
:PROPERTIES:
:CUSTOM_ID: plugin-contract
:END:
*** Mandatory: Plugin Info
:PROPERTIES:
:CUSTOM_ID: mandatory-plugin-info
:END:
...
:PROPERTIES:
:CUSTOM_ID: optional-lifecycle-functions
:END:
#+begin_src C
...
:PROPERTIES:
:CUSTOM_ID: optional-hook-functions
:END:
...
:PROPERTIES:
:CUSTOM_ID: timing-fields-microseconds
:END:
| Field               | Description                        |
|---------------------+------------------------------------|
...
:PROPERTIES:
:CUSTOM_ID: inter-plugin-communication
:END:
...
:PROPERTIES:
:CUSTOM_ID: plugin-execution-order
:END:
